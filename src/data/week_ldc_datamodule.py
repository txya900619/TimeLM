    ...
    def __getitem__(self, index):
        ...
        with open(self.json_list_file_path, encoding="utf-8") as data_file:
            ...
            if random.random() >= self.no_date_prob:  # nosec
                week_token = (
                    datetime.datetime.strptime(date, "%Y%m%d").date() - datetime.date(1991, 1, 1)
                ).days // 7 + 1

            tokens_bos = torch.LongTensor([self.bos] + tokens)
            tokens_eos = torch.LongTensor(tokens + [self.eos])
            return (tokens_bos, tokens_eos, week_token)


class WeekLDCDataModule(LightningDataModule):
    def __init__(
        self,
    ...
    def setup(self, stage: str) -> None:
        if not self.data_train and not self.data_val and not self.data_test:
            ...
            self.data_train = WeekLDCDataset(self.train_json_list_file_path, self.bos, self.eos)
            self.data_val = WeekLDCDataset(self.valid_json_list_file_path, self.bos, self.eos)
            self.data_test = WeekLDCDataset(self.test_json_list_file_path, self.bos, self.eos)
