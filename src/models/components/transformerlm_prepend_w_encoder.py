...
class TransformerLM(nn.Module):
    ...
    def forward(self, x, hx=None):
        ...
        date_contexts = date_contexts + self.d_embedding_positional_embedding(
            torch.arange(self.date_context_range, device=date_contexts.device)
        ).unsqueeze(0).repeat(date_contexts.size(0), 1, 1)

        date_contexts = self.d_embedding_encoder(
            date_contexts,
            mask=date_contexts_mask,
            src_key_padding_mask=date_contexts_padding_mask,
            is_causal=True,
        )
    ...
    def get_predict_future(self, x):
        _, date_contexts = x
        date_contexts = date_contexts.unsqueeze(1)
        max_date = self.date_embedding.num_embeddings - 1
        if date_contexts.max() > max_date:
            print(" > max_date auto clamped")
        date_contexts = date_contexts.clamp(min=0, max=max_date)
        ...
        date_contexts = date_contexts[:, : self.date_context_range - 1]
