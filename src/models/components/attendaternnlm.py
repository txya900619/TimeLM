...
class AttentionEmbedding(nn.Module):
    def __init__(self, num_date_embeddings, context_dim, nhead, dropout, look_ahead=7):
        super().__init__()
    ...
    def forward(self, x: Tensor):  # x: (B,)
        x = x.unsqueeze(1)  # (B, 1)
        x = x.repeat(1, self.look_ahead)  # (B, look_ahead)
        x = x - torch.arange(self.look_ahead, device=x.device).flip(0)  # (B, look_ahead)
        x = x.clamp(min=0)  # (B, look_ahead)

        x = self.emb(x)  # (B, look_ahead, context_dim)
        old, now = torch.split(
            x, (self.look_ahead - 1, 1), dim=1
...
class DateRNNLM(nn.Module):
    ...
    def __init__(
        ...
        self.date_embedding = AttentionEmbedding(
            num_date_embeddings, context_dim, nhead, dropout, look_ahead=7
        )
