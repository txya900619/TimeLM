...
class AttentionEmbedding(nn.Module):
    def __init__(self, num_date_embeddings, context_dim, nhead, dropout, look_forward=7):
        super().__init__()
    ...
    def forward(self, x: Tensor):  # x: (B,)
        if self.training:
            x = x.unsqueeze(1)  # (B, 1)
            x = x.repeat(1, self.look_forward)  # (B, look_ahead)
            x = x + torch.arange(self.look_forward, device=x.device)  # (B, look_ahead)
            x = x.clamp(max=self.num_date_embeddings - 1)  # (B, look_ahead)

            x = self.emb(x)  # (B, look_ahead, context_dim)
            x, _ = self.atten(x, x, x)  # (B, look_ahead, context_dim)
            x = x[:, 0, :]  # (B, context_dim)
        else:
            x = self.emb(x)  # (B, context_dim)
        return x
...
class DateRNNLM(nn.Module):
    ...
    def __init__(
        ...
        self.date_embedding = AttentionEmbedding(
            num_date_embeddings, context_dim, nhead, dropout, look_forward=7
        )
